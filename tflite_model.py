...
class Model(object):
  ...
  def infer(self, samples):
    ...
    frame_logits = self._interpreter.get_tensor(self._output_index['frame_logits'])
    onset_logits = self._interpreter.get_tensor(self._output_index['onset_logits'])
    velocity_values = self._interpreter.get_tensor(self._output_index['velocity_values'])
    
    # Check if offset_logits exists and has valid shape
    offset_logits = self._interpreter.get_tensor(self._output_index['offset_logits'])
    if offset_logits.ndim == 0:  # Empty tensor, create zeros with correct shape
        offset_logits = np.zeros_like(frame_logits)
